...
const sketch = function(p) {
  ...
  p.setup = function() {
    ...
    const containerSize = $('.sketchpad').getBoundingClientRect();
    ...
    let shapesWrapper = $('.shapes-bar-wrapper');
    let shapesFragment = new DocumentFragment();

    Object.keys(supportedModels).forEach(  model => {

      let btn  = document.createElement('button');
      btn.className = `btn-shape btn-shape-${model}`
      btn.innerText = supportedModels[model]
    ...
    shapesWrapper.innerHTML = '';
    shapesWrapper.append(shapesFragment)
    initModel('bee');
    $(".btn-shape-bee").classList.toggle('selected');
    
    $(".btn-clear").addEventListener('click', restart);
  ...
  */
  ...
  function initModel(modelName) {
    ...
    model = new ms.SketchRNN(`${BASE_URL}${modelName}.gen.json`);
